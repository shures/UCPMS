...
var DropDown = function (_a) {
    ...
    return (React.createElement("div", { className: "drop-down" },
        React.createElement("div", { className: "option-wrapper" },
            React.createElement("ul", null, options.map(function (option, index) { return (React.createElement("li", { key: index, className: option.value === value ? "active" : "", onClick: function () {
                    onSelect(option);
                } }, option.label)); })))));
};
...
var MonthPicker = function (_a) {
    ...
    var currentLocale = useMemo(function () { return getConfig("currentLocale"); }, [getConfig]);
    ...
    return (React.createElement("div", { className: "control month" },
        React.createElement("span", { className: "current-month", onClick: function () { return setShowDropdown(!showDropdown); } }, currentMonth.label),
        showDropdown && React.createElement(DropDown, { options: monthList, value: currentMonth.value, onSelect: handleDropdownView })));
};
...
var YearPicker = function (_a) {
    ...
    return (React.createElement("div", { className: "control year" },
        React.createElement("span", { className: "current-year", onClick: function () { return setShowDropdown(!showDropdown); } }, currentYear.label),
        showDropdown && React.createElement(DropDown, { options: years, value: currentYear.value, onSelect: handleDropdownView })));
};
...
var CalenderController = function (props) {
    ...
    return (React.createElement("div", { className: "calendar-controller" },
        React.createElement("span", { className: "control icon", title: "prev", onClick: onPreviousMonth },
            React.createElement(Previous, null)),
        React.createElement("div", { className: "date-indicator" },
            React.createElement(MonthPicker, { date: calenderDate, onSelect: onMonthSelect }),
            React.createElement(YearPicker, { date: calenderDate, onSelect: onYearSelect })),
        React.createElement("span", { className: "control icon icon-today", title: "today", onClick: onToday },
            React.createElement(Today, { color: "#2096f5" })),
        React.createElement("span", { className: "control icon", title: "next", onClick: onNextMonth },
...
var DayPickerBody = function (_a) {
    ...
    var weeksInMonth = useMemo(function () { return Math.ceil((date.firstAdDayInBSMonth.getDay() + date.numberOfDaysInBSMonth) / 7) - 1; }, [date]);
    var previousMonth = useMemo(function () { return (date.bsMonth - 1 !== 0 ? date.bsMonth - 1 : 12); }, [date]);
    var previousYear = useMemo(function () { return (previousMonth === 12 ? date.bsYear - 1 : date.bsYear); }, [previousMonth, date]);
    ...
    return (React.createElement("tbody", null, range(0, weeksInMonth).map(function (weekNum) { return (React.createElement("tr", { key: weekNum }, range(1, 7).map(function (weekDayNum) {
        ...
        return (React.createElement("td", { key: weekDayNum, className: "month-day " + (dayInfo.isCurrentMonth ? "current" : "disabled") + " " + (dayInfo.isToday ? "today" : "") + " " + (dayInfo.isSelected ? "selected" : ""), onClick: function () { return onDateSelectHandler(dayInfo); } }, dayInfo.day));
    }))); })));
};
...
var DayPicker = function (_a) {
    ...
    return (React.createElement("table", null,
        ...
        React.createElement(DayPickerBody, { selectedDate: selectedDate, calenderDate: calenderDate, onSelect: onDaySelect })));
};
...
var Calender = function (_a) {
    ...
    var _c = useState(parsedDateInitialValue), selectedDate = _c[0], setSelectedDate = _c[1];
    var _d = useState(parsedDateInitialValue), calenderDate = _d[0], setCalenderDate = _d[1];
    var _e = useState(false), daySelected = _e[0], setDaySelected = _e[1];
    useEffect(function () {
        if (isInitialized) {
    ...
    return (React.createElement("div", { className: "nepali-date-picker" },
        React.createElement("div", { className: "calendar-wrapper" }, isInitialized && (React.createElement(Fragment, null,
            React.createElement(CalenderController, { onPreviousMonth: onPreviousMonthHandler, onNextMonth: onNextMonthClickHandler, calenderDate: calenderDate, onToday: onTodayClickHandler, onYearSelect: onYearSelectHandler, onMonthSelect: onMonthSelectHandler }),
            React.createElement(DayPicker, { selectedDate: selectedDate, calenderDate: calenderDate, onDaySelect: onDaySelectHandler }))))));
};
...
var NepaliDatePicker = function (_a) {
    ...
    var _b = useState(false), showCalendar = _b[0], setShowCalendar = _b[1];
    var dateEl = useRef(null);
    var today = ADToBS(new Date());
    var _c = useState(defaultDate === true ? today : defaultDate), value = _c[0], setValue = _c[1];
    ...
    return (React.createElement(ConfigProvider, null,
        React.createElement("div", { className: "input-group", id: "dateInputGroup" },
            React.createElement("input", { type: "text", className: className, readOnly: true, ref: dateEl, value: value || "", disabled: disabled, onClick: function () { return setShowCalendar(!showCalendar); }, placeholder: placeholder }),
            showResetDateButton && (React.createElement("div", { className: "input-group-append" },
                React.createElement("button", { className: "btn btn-outline-secondary", type: "button", id: "resetDateButton", onClick: function () { return clearDate(defaultDate); } }, resetButtonElement)))),
        showCalendar && React.createElement(Calender, { value: value || ADToBS(new Date()), onChange: handleOnChange })));
};
